...
class MenuAdministrador(tk.Toplevel):
    ...
    def agregar_mas_widgets(self):
        ...
        boton_salir = ttk.Button(frame_principal, text="Salir", command=self.salir)
        boton_salir.grid(row=0, column=2, sticky="e")

        boton_g_admin = ttk.Button(frame_principal, text = "Gestionar administradores", command=self.abrir_ventana_g_admin)
        boton_g_admin.grid(row=1, column=0, sticky="nsew")
        boton_g_colab = ttk.Button(frame_principal, text = "Gestionar colaboradores", command=self.abrir_ventana_g_colab)
        boton_g_colab.grid(row=1,column=1,sticky="nsew")
        boton_g_prof = ttk.Button(frame_principal, text = "Gestionar profesores", command=self.abrir_ventana_g_prof)
        boton_g_prof.grid(row=1,column=2,sticky="nsew")

        boton_g_ciclos = ttk.Button(frame_principal, text="Gestionar ciclos", command=self.abrir_ventana_g_ciclos)
        boton_g_ciclos.grid(row=2, column=0, sticky="nsew")
        boton_g_especialidades = ttk.Button(frame_principal, text="Gestionar especialidades", command=self.abrir_ventana_g_especialidades)
        boton_g_especialidades.grid(row=2, column=1, sticky="nsew")
        boton_asg_ciclos = ttk.Button(frame_principal, text="Asignar ciclos a profesores", command=self.abrir_ventana_asg_ciclos)
        boton_asg_ciclos.grid(row=2, column=2, sticky="nsew")

        boton_r_estud = ttk.Button(frame_principal, text="Registro de estudiantes", command=self.abrir_ventana_r_estud)
        boton_r_estud.grid(row=3, column=0, sticky="nsew")
        boton_n_estud = ttk.Button(frame_principal, text="Notas Estudiantes", command=self.abrir_ventana_n_estud)
        boton_n_estud.grid(row=3, column=1, sticky="nsew")
